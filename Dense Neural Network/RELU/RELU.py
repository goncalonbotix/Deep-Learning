...
learningRate = 0.005; # RELU Activation Function can increase much faster, so learning rate on the gradients must be smaller. 
...
for n in range(0,numEpochs):
    ...
    for i in range(trainX.shape[0]):   # will go through one image per iteration. 
            ...
            a1 = np.maximum(0,s1) # np.maximum hopefully operates on the array
            s2 = np.dot(w2,a1) + b2
            a2 = np.maximum(0,s2) 
            ...
            dev_relu = a2.copy()
            dev_relu[dev_relu < 0]=0
            dev_relu[dev_relu > 0]=1

            delta2 = -np.multiply(trainY[i]-a2,dev_relu)

            dev_relu1 = a1.copy()
            dev_relu1[dev_relu1 < 0]=0
            dev_relu1[dev_relu1 > 0]=1

            delta1 = np.multiply(np.dot(np.transpose(w2),delta2), dev_relu1)
